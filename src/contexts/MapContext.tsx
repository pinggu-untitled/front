...
export const MapProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const getSubPostInfo = (
    ...
    const infoPosition = new kakao.maps.LatLng(
      (markPosition.getLat() + 0.001).toFixed(6),
      markPosition.getLng()
    );
  ...
  const getSubPosts = (map: kakao.maps.Map | null) => {
    ...
    const [swLat, swLng] =
      bounds?.getSouthWest().toString().slice(1, -1).split(',') ?? [];
    const [neLat, neLng] =
      bounds?.getNorthEast().toString().slice(1, -1).split(',') ?? [];
    ...
    fetcher(
      ...
      .then((posts) => {
        ...
        posts.forEach((post: IPost) => {
          ...
          subMarker.addListener('mouseover', () =>
            getSubPostInfo(subMarker, map, post, position)
          );
